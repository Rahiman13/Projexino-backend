...
const getScheduledNewsletters = async (req, res) => {
    try {
        ...
        const newslettersWithIST = newsletters.map(newsletter => {
            ...
            return {
                ...newsletter.toObject(),
                scheduledTimeIST: scheduledForIST // formatted time in IST
            };
        });
...
// Controller to get the total count of all newsletter announcements
const getTotalAnnouncementCount = async (req, res) => {
    try {
        const totalCount = await Newsletter.countDocuments(); // Get the total count of newsletters
        return res.status(200).json({ totalCount });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ error: 'Error fetching total announcement count' });
    }
};

// Controller to get all newsletters
const getAllNewsletters = async (req, res) => {
    try {
        const newsletters = await Newsletter.find(); // Retrieve all newsletters from the database
