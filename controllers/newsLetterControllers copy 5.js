...
const scheduleBlogNewsletter = async (req, res) => {
    try {
        ...
        // Store blog data in the newsletter metadata
        const blogData = selectedBlogs.map(blog => ({
            id: blog._id,
            title: blog.title,
            slug: blog.slug,
        ...
        const newsletter = new Newsletter({
            ...
            metadata: {
                ...
                blogData: blogData, // Store blog data in metadata
        ...
        if (scheduledTime > 0) {
            setTimeout(async () => {
                try {
                    ...
                    // Generate HTML content using stored blog data
                    const content = `
                        <!DOCTYPE html>
                        <html>
                            <head>
                                ...
                                <div class="wrapper">
                                    <div class="container">
                                        <div class="header">
                                            ...
                                            ${newsletter.metadata.additionalContent ? `<p>${newsletter.metadata.additionalContent}</p>` : ''}
                                        </div>
                                        
                                        <div class="blog-grid">
                                            ${newsletter.metadata.blogData.map(blog => `
